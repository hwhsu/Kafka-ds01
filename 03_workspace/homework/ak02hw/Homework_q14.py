...
'''
 * Tips:
 * 由於14題主要是By每一個unique的商品貨物編號算出它的count,我們可以利用一個"Dict(key是part_no, value是record count)"
 * 的資料容器物件來快速達成目的
 *
...
if __name__ == '__main__':
    ...
    # 產生一個 Map: key是part_no, value是record count <---- 存放 "題目#14" 答案的容器
    parts_transCount = defaultdict(int)
        ...
        while True:
            ...
            for record in records:
                ...
                if record.error():
                    ...
                    # Use defaultdict to count cumulatively
                    parts_transCount[msgKey] += 1

                    # *** 你要開發的Block在這裡 [End] ***

            # 打印出最更新的結果 <---- "題目#14" 的答案
            print('Parts trans_count: ' + '|'.join(f'{k}={v}' for k, v in OrderedDict(sorted(parts_transCount.items())).items()))
